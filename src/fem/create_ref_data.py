        ...
        for i in range(neval):
            for j in range(neval):
                ...
                self.evaluation_points[p_index,:] = np.array((
                    [ref_data_x.evaluation_points[j], 
                     ref_data_y.evaluation_points[i]]))
                self.quadrature_weights[p_index] = ref_data_x.quadrature_weights[j] * ref_data_y.quadrature_weights[i]
                
                for k in range(deg[0]+1):
                    for l in range(deg[1]+1):
                        
                        f_index = k + (deg[0]+1) * l
                        self.reference_basis[f_index, p_index] = ref_data_x.reference_basis[k, i] * ref_data_y.reference_basis[l, j]
                        self.reference_basis_derivatives[f_index, p_index, :] = np.array((
                            [ref_data_x.reference_basis_derivatives[k, i] * ref_data_y.reference_basis[l, j],
                            ref_data_x.reference_basis[k, i] * ref_data_y.reference_basis_derivatives[l, j]]
                        ))
