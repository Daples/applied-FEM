...
problem_L = lambda x, Nj, dxNj, dyNj: np.multiply(Nj, np.multiply(np.sin(x[:,0]), np.sin(x[:,1]))) 

fe_geometry, fe_space = read_mat("data/distressed_robotDN.mat")
...
for current_element in range(fe_geometry.m):
    ...
    plt.contourf(plot_grid_x, plot_grid_y, plot_grid_u, cmap ='viridis', vmin=-0.5, vmax=0.5)
    plt.clim(-0.5, 0.5) 
